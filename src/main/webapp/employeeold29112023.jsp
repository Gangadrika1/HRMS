...
%>
 ...
 // Check if the old records per page is not equal to the new records per page
 var isRecordsPerPageChanged = <%= currentPage != newRecordsPerPage %>;

 // Check if a search has been invoked
 var isSearchInvoked = <%= searchInvoked %>; // You should replace this with a corresponding variable

 // Check if the script should be executed
 if (isRecordsPerPageChanged && !isSearchInvoked) {
var recordsPerPage = <%= newRecordsPerPage %>; // Use Java variable in JavaScript
 var currentPage = <%= currentPage %>; 
...
<!--                                 <a href="#" class="btn add-btn" data-toggle="modal" data-target="#editEmployee"><i class="fa fa-plus"></i>Edit </a> -->
    ...
    <% if (pageno > 1) { %>
        <a href="employee.jsp?page=<%=pageno - 1%>&recordsPerPage=<%= newRecordsPerPage %>">Previous</a>
    ...
    <% for (int i = 1; i <= noOfPages; i++) { %>
        ...
        <% } else { %>
            <%-- <a href="employee.jsp?page=<%=i%>"><%=i%></a> --%>
            <a href="employee.jsp?page=<%= i %>&recordsPerPage=<%= newRecordsPerPage %>"><%= i %></a>
        <% } %>
    <% } %>

    <% if (pageno < noOfPages) { %>
        <a href="employee.jsp?page=<%=pageno + 1%>&recordsPerPage=<%= newRecordsPerPage %>">Next</a>
    <% } %>
