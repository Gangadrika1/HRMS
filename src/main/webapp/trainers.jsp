                ...
                <div class="row filter-row">
                   ...
                   <div class="col-sm-6 col-md-3">  
				      <div class="form-group form-focus">
					   <input name="email" id="email" type="text" class="form-control floating">
                    ...
                    <div class="col-sm-6 col-md-3">  
				      <div class="form-group form-focus">
					   <input name="TrainersID" id="TrainersID" type="text" class="form-control floating">
					   <label class="focus-label">Trainers ID</label>
				     </div>
				    </div>
...
<div class="row justify-content-center align-items-center" id = "flag1">
      <% if (request.getParameter("email") ==null && request.getParameter("TrainersID") ==null) { %>
    <% if (pageno > 1) { %>
        <a href="trainers.jsp?page=<%=pageno - 1%>&newRecordsPerPage=<%= newRecordsPerPage %>">Previous</a>
    <% } %>
    <% for (int i = 1; i <= noOfPages; i++) { %>
        <% if (i == pageno) { %>
            <%=i%>
        <% } else { %>
            <a href="trainers.jsp?page=<%=i%>&newRecordsPerPage=<%= newRecordsPerPage %>"><%="&nbsp;&nbsp;&nbsp;" + i + "&nbsp;&nbsp;"%></a>
        <% } %>
    <% } %>
    <% if (pageno < noOfPages) { %>
        <a href="trainers.jsp?page=<%=pageno + 1%>&newRecordsPerPage=<%= newRecordsPerPage %>">Next</a>       
    <% } %>
	
	<% }else { %>
	<!-- after search it will execute -->
	 <% if (pageno > 1) { %>
	        <a href="trainers.jsp?page=<%=pageno - 1%>&newRecordsPerPage=<%= newRecordsPerPage %>&email=<%=request.getParameter("email")%>&TrainersID=<%=request.getParameter("TrainersID")%>">Previous</a>
	    <% } %>
	    ...
	    <% for (int i = 1; i <= noOfPages; i++) { %>
	        <% if (i == pageno) { %>
	            <%=i%>
	        <% } else { %>
	            <a href="trainers.jsp?page=<%=i%>&newRecordsPerPage=<%= newRecordsPerPage %>&email=<%=request.getParameter("email")%>&TrainersID=<%=request.getParameter("TrainersID")%>"><%=i%></a>
	        <% } %>
	    <% } %>
	
	    <% if (pageno < noOfPages) { %>
	        <a href="trainers.jsp?page=<%=pageno + 1%>&newRecordsPerPage=<%= newRecordsPerPage %>&email=<%=request.getParameter("email")%>&TrainersID=<%=request.getParameter("TrainersID")%>">next</a>
	    <% } %>
